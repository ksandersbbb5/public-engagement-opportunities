// pages/api/find-public-events.js
...
function parseUSDate(label) {
  // Accepts "Month Day, Year" and returns Date or null
...
- Include contact email when listed on the page; otherwise leave null.`;
    ...
    try {
      ...
      // If not enough data, return empty (no fabricated fallback)
      if (total < 15) {
        results = emptyResult();
      }
    } catch (err) {
